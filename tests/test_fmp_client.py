...
class FMPClientTests(unittest.TestCase):
    ...
    def test_collects_raw_fmp_fundamentals(self) -> None:
        def fake_get(url, params=None, timeout=None):  # noqa: ANN001
            response = MagicMock()
            if url.endswith("/v3/profile/INVE-B.ST"):
            ...
            elif url.endswith("/v3/income-statement/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{"revenue": 100, "eps": 12.3, "freeCashFlow": 50}]
            elif url.endswith("/v3/balance-sheet-statement/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{"totalDebt": 30}]
            elif url.endswith("/v3/cash-flow-statement/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{"netCashProvidedByOperatingActivities": 80, "freeCashFlow": 50}]
            elif url.endswith("/v3/ratios/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{
                    "returnOnEquity": 0.21,
                    "returnOnAssets": 0.14,
            ...
            elif url.endswith("/v3/key-metrics/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{
                    "eps": 12.3,
            ...
            elif url.endswith("/v3/financial-growth/INVE-B.ST"):
                response.status_code = 200
                response.json.return_value = [{"revenueGrowth": 0.08, "epsgrowth": 0.11, "freeCashFlowGrowth": 0.09}]
            else:
        ...
        with patch.dict(os.environ, {"FMP_API_KEY": "test-key"}, clear=True), patch(
