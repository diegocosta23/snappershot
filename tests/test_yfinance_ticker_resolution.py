...
class YahooFinanceTickerResolutionTests(unittest.TestCase):
    def test_resolves_display_names_before_querying_yfinance(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                "src.snappershot.collectors.yfinance_client.yf.Ticker"
            ) as ticker_cls:
                ...
                client = YahooFinanceClient(
                    symbol_resolver=SymbolResolver(cache_path=cache_path)
                )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with patch(
                ...
                ticker_instance.dividends = pd.Series(
                    [1.25, 1.5], index=pd.to_datetime(["2025-01-01", "2025-06-01"])
                )
                ticker_cls.return_value = ticker_instance
                ...
                self.assertEqual(
                    raw["company"]["name"], {"value": "ABB Ltd", "source": "yfinance"}
                )
                self.assertEqual(
                    raw["valuation"]["pe"], {"value": 20, "source": "yfinance"}
                )
                self.assertEqual(
                    raw["profitability"]["gross_margin"],
                    {"value": 0.4, "source": "yfinance"},
                )
                self.assertEqual(
                    raw["growth"]["revenue_growth"],
                    {"value": 0.12, "source": "yfinance"},
                )
                self.assertEqual(
                    raw["balance_sheet"]["total_assets"],
                    {"value": 9000000, "source": "yfinance"},
                )
                self.assertEqual(
                    raw["cashflow"]["operating_cash_flow"],
                    {"value": 4000000, "source": "yfinance"},
                )
                self.assertEqual(
                    raw["dividends"]["latest_dividend"],
                    {"value": 1.5, "source": "yfinance"},
                )
                self.assertEqual(
                    raw["market_data"]["current_price"],
                    {"value": 200, "source": "yfinance"},
                )
