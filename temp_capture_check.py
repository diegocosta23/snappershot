...
result = asyncio.run(engine.run('ABB.ST', screenshots=[Path('C:/temp/placeholder.png')]))
...
print('ROWS', {t: cur.execute(f'SELECT COUNT(*) FROM {t}').fetchone()[0] for t in ['stocks','fundamentals','prices','technical_indicators','news','screenshots','analysis_results']})
conn.close()
